...
def get_points(my_data, symbol, right):
    ...
    no_timestamp_marker = {'color': f'{no_timestamp_color}', 'size': 5, 'symbol': 'square'}
    ...
    return {
        'data': trace,
        'layout': go.Layout(
            title=f'Daily bars by expiry date - {symbol} {right}',
            showlegend=True,
            legend=go.Legend(
                x=0,
                y=1.0
...
def get_data(right, symbol):
    query = 'select c.right, c.strike, c."lastTradeDateOrContractMonth"::date, c."cantGetDailyBars", ' \
            'exists (select 1 from contract_daily_bars b where c."conId" = b."conId") as has_bars, ' \
...
@app.callback(Output('my-bars-by-symbol-graph', 'figure'), [Input('right', 'value'),
                                                            Input('symbol', 'value'),
                                                            Input('interval-component', 'n_intervals')])
def update_bars_figure(right, symbol, n_intervals):
