...
def get_data(right, symbol, expiry_date, strike):
    query = 'select c.*, b.*, b."dailyBarId" is not null as has_bars  ' \
            ...
            'order by b.date '.format(strike, symbol, right, expiry_date)

    my_data = pd.read_sql(query, connection, parse_dates={"date": {"utc": True}})
    if any(my_data.has_bars):
        my_data.strike = my_data.strike.astype(float)
        my_data = my_data.set_index('date')
        daily_data = my_data[['open', 'high', 'low', 'close', 'volume']]
        ohlc_dict = {'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum'}
        daily_data = daily_data.resample('1D').apply(ohlc_dict).dropna(how='any')
        return daily_data
    else:
        return None
...
@app.callback(Output('strike', 'options'),
              [Input('right', 'value'),
               Input('symbol', 'value'),
               Input('interval-component', 'n_intervals'),
               Input('date-picker', 'date')])
def get_strikes(right, symbol, n_intervals, date):
    query = 'select distinct c.strike, b."dailyBarId" is not null as has_bars ' \
            'from contracts c left join contract_daily_bars b on c."conId" = b."conId" ' \
...
def update_bars_figure(right, symbol, n_intervals, date, strike):
    ...
    if isinstance(my_data, pd.DataFrame) and not my_data.empty:
        ...
        layout['title'] = f'Daily bars - {date_string} {symbol} {strike} {right} '
