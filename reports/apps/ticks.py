...
def get_data(right, symbol, expiry_date, strike, bar_size):
    query = 'select t.price, t.size, t.time  ' \
    ...
    bar_data = my_data.copy()
    bar_data = bar_data.set_index('time')
    ohlc_dict = {'price': ['first', 'max', 'min', 'last'], 'size': 'sum'}
    bar_data = bar_data.resample(bar_size).apply(ohlc_dict).dropna(how='any')
...
layout = html.Div(className='container',
                  children=[
                      ...
                      html.Div([
                          html.Label("Bar Size:", htmlFor="barSize", className='form-check-label'),
                          dcc.Dropdown(
                              id='bar-size',
                              className='form-check-inline form-check-input',
                              clearable=False,
                              options=[{'label': '1 S', 'value': '1 S'},
                                       {'label': '30 S', 'value': '30 S'},
                                       {'label': '1 Min', 'value': '1 T'},
                                       {'label': '5 Min', 'value': '5 T'},
                                       {'label': '15 Min', 'value': '15 T'},
                                       {'label': '30 Min', 'value': '30 T'},
                                       {'label': '1 H', 'value': '1 H'},
                                       {'label': '2 H', 'value': '2 H'},
                                       {'label': '1 D', 'value': '1 D'},
                                       {'label': '1 W', 'value': '1 W'},
...
def get_points(my_data, symbol, right, selected_date, strike):
    ...
    trace = [
        ...
        go.Candlestick(
            x=my_data.index,
            open=my_data['price']['first'],
            high=my_data['price']['max'],
            low=my_data['price']['min'],
            close=my_data['price']['last'],
            name='Price',
            xaxis='x1',
            yaxis='y1'
...
def update_bars_figure(right, symbol, n_intervals, date, strike, bar_size):
    fig = plotly.tools.make_subplots(rows=3, cols=1, specs=[[{'rowspan': 2}], [{}], [{}]], vertical_spacing=0.01,
                                     shared_xaxes=True,
                                     shared_yaxes=False)
    layout = fig['layout']
    my_data = get_data(right, symbol, date, strike, bar_size)
