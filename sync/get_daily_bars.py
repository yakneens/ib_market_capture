...
def parse_args():
    ...
    help_str = "Start cutoff."
    my_parser.add_argument("-s", "--start_cutoff", dest="start_cutoff",
                           help=help_str,
                           required=True)

    help_str = "End cutoff."
    my_parser.add_argument("-e", "--end_cutoff", dest="end_cutoff",
                           help=help_str,
                           required=True)

    help_str = "How many days ago was the last load."
    my_parser.add_argument("-l", "--last_load", dest="last_load",
                           help=help_str,
                           required=True)

    help_str = "Can't get bars flag."
    my_parser.add_argument("-t", "--cant_get_bars", dest="cant_get_bars",
                           help=help_str,
                           choices=['true', 'false'],
                           required=True)
    help_str = "Date order."
    my_parser.add_argument("-o", "--date_order", dest="date_order",
                           help=help_str,
                           choices=['asc', 'desc', 'ASC', 'DESC'],
                           required=True)

    # common_args_parser.set_defaults(func=get_first_trade_date_command)
...
def set_cant_get_daily_bars_flag(conId):
    result = connection.execute(
        contract_table.update().where(contract_table.c.conId == conId).values(cantGetDailyBars=True))
...
def main(args):
    ...
    cant_get_bars = args.cant_get_bars
    ...
    date_query = 'select distinct c."lastTradeDateOrContractMonth"::date from contracts c join contract_ib_first_timestamp t on c."conId" = t."contractId" ' \
                 ...
                 'order by c."lastTradeDateOrContractMonth"::date {} '.format(cant_get_bars, start_cutoff, end_cutoff,
                                                                              date_order)
        ...
        for index, row in con_df.iterrows():
            ...
            try:
                my_bars = ib.reqHistoricalData(my_con, endDateTime='', durationStr='{} D'.format(num_days),
                                               barSizeSetting='8 hours', whatToShow='TRADES', useRTH=False,
                                               formatDate=2)
