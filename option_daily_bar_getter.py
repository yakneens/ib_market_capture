...
class OptionDailyBarGetter:
    ...
    def set_cant_get_daily_bars_flag(conId):
        contract_table = db.meta.tables["contracts"]
        result = db.engine.execute(contract_table.update().where(contract_table.c.conId == conId).values(cantGetDailyBars=True))


    def to_df(self, my_bars, conId):
    ...
    def save_to_db(self, bars, conId):
        ...
        result = db.engine.execute(self.contract_table.update().where(self.contract_table.c.conId == conId).values(
